...
def parse_txt(file_content):
    ...
    for block in blocks:
        flight_no = re.search(r"(BA\d{3,4})", block)
        aircraft = re.search(r"\n(\w{3})\n", block)
        route = re.search(r"(LHR\w{3})", block)
        etd = re.search(r"STD: \d{2} Apr - (\d{2}:\d{2})z", block)
        load = re.search(r"(\d{1,3})%Status", block)

        if flight_no and aircraft and route and etd and load:
            flights.append({
                "Flight Number": flight_no.group(1),
                "Aircraft Type": aircraft.group(1),
                "Route": route.group(1),
                "ETD": etd.group(1) + "z",
                "Load Factor Numeric": int(load.group(1))
            })

    df = pd.DataFrame(flights)

    def subtract_35_minutes(etd_str):
        etd = datetime.strptime(etd_str.replace("z", ""), "%H:%M")
