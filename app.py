...
# Define the PDF generator
class BA_PDF(FPDF):
    def header(self):
        self.image("ba_logo.png", 10, 6, 30)
...
def parse_txt(file_content):
    ...
    df = pd.DataFrame(matches, columns=["Flight Number", "Aircraft Type", "Route", "ETD", "Load Factor"])
    ...
    df["ETD"] = df["ETD"] + "z"
    df["Load Factor Numeric"] = df["Load Factor"].str.rstrip('%').astype(int)
